...
public class RunTest {
	...
	/*
	 * JDBC를 사용하기 위해서는 '자바 프로젝트'에 JDBC드라이버를 추가해 줘야한다.
	 * 
	 * *JDBC용 객체 
	 * - Connection : BC의 연결정보를 담고있는 객체
	 ...
	 * 3) Statement 객체 생성 : Connection객체를 이용해서 생성(sql문 실행 및 결과를 받는 객체)
	...
	/*
		...
		// 실행할 sql문(완전한 상태로 만들어두기)(문자열의 맨 뒤에 세미콜론 없어야된다!!!!!!)
		String sql = "INSERT INTO TEST VALUES(" + num + ", '" + name + "', SYSDATE)";
		
		try {
			// 1) JDBC Driver등록
			Class.forName("oracle.jdbc.driver.OracleDriver"); // 오류 발생 시 ojdbc6.jar 파일을 추가하지 않았는지 확인, 또는 오타
			...
			try {
				//7) 다 사용한 JDBC용 객체를 반드시 자원 반납(생성된 역순으로)
				stmt.close();
				conn.close();
			} catch (SQLException e) {
				e.printStackTrace();
		...
		*/
		...
		// 2. 내 pc DB상의 JDBC계정에 TEST테이블에 모든 데이터 조회해보기
		...
		try {
			...
			while(rset.next()) {
				//현재 참조하는 rset으로부터 어떤 칼럼에 해당하는 값을 어떤 타입으로 추출할건지 제시해줘야한다.
